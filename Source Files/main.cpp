...
int main(int argc, char** argv) {
	...
	auto savePath = std::filesystem::path("D:\\FIT_Apprenti_Vague_006\\ER Saves\\ER0000.sl2");																																															// { util::FindFileInSubDirectory(fmt::format("{}/.steam/steam/steamapps/compatdata/1245620/pfx/drive_c/users/steamuser/AppData/Roaming/EldenRing", util::GetEnvironmentVariable("HOME")), "ER0000.sl2") };
	...
	*/
	SaveFile saveFile{ savePath };
	std::cout << "using savefile '" << savePath.string() << "'\nSteam ID embedded in the savefile: " << saveFile.steamId() << std::endl;

	saveFile.printActiveSlots();
	saveFile.debugListItems(0);
	auto slot_ = saveFile.slots[2];

	auto md5 = util::GenerateMd5(&slot_.SlotSection());


	//saveFile.copySlot(2, 3);
